...
/*!
 * Wake Up time of the Radio.
 */
static uint32_t RadioWakeUpTime;
...
/* GIMASI ADD ON - add TimeStamp of Join Procedure - to calculate correctly BackOff on successive Joins */
TimerTime_t LoraMacFirstJoinTimeStamp;
...
static void RxWindowSetup( uint32_t freq, int8_t datarate, uint32_t bandwidth, uint16_t timeout, bool rxContinuous )
{
    ...
    if( Radio.GetStatus( ) == RF_IDLE )
    {
        ...
        {
            modem = MODEM_FSK;
            Radio.SetRxConfig( modem, 50000, downlinkDatarate * 1000, 0, 83333, 5, 0, false, 0, true, 0, 0, false, rxContinuous );
        }
        else
        {
            modem = MODEM_LORA;
...
static LoRaMacStatus_t ScheduleTx( )
{
    ...
    // Useful CB for LED UI
    if ( LoRaMacCallbacks->SendOnChannelCB)
    	LoRaMacCallbacks->SendOnChannelCB(dutyCycleTimeOff);
...
//    TimerTime_t timeElapsed = TimerGetElapsedTime( 0 );
    TimerTime_t timeElapsed = TimerGetElapsedTime( LoraMacFirstJoinTimeStamp );
...
static void CalculateBackOff( uint8_t channel )
{
    ...
    if( IsLoRaMacNetworkJoined == false )
    {
        ...
        if( TxTimeOnAir > 0 )
        {
           // Disable rndTimeOff duting Join Procedure
           rndTimeOff = true;
        }
    }
...
LoRaMacStatus_t PrepareFrame( LoRaMacHeader_t *macHdr, LoRaMacFrameCtrl_t *fCtrl, uint8_t fPort, void *fBuffer, uint16_t fBufferSize )
{
    ...
    switch( macHdr->Bits.MType )
    {
        case FRAME_TYPE_JOIN_REQ:
            RxWindow1Delay = LoRaMacParams.JoinAcceptDelay1 - RadioWakeUpTime; //RADIO_WAKEUP_TIME;
            RxWindow2Delay = LoRaMacParams.JoinAcceptDelay2 - RadioWakeUpTime; //RADIO_WAKEUP_TIME;
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_UP:
            ...
            RxWindow1Delay = LoRaMacParams.ReceiveDelay1 - RadioWakeUpTime; //RADIO_WAKEUP_TIME;
            RxWindow2Delay = LoRaMacParams.ReceiveDelay2 - RadioWakeUpTime; //RADIO_WAKEUP_TIME;
...
LoRaMacStatus_t SendFrameOnChannel( ChannelParams_t channel )
{
    ...
    { // High Speed FSK channel
        ...
        Radio.SetTxConfig( MODEM_FSK, txPower, 25000, 0, datarate * 1000, 0, 5, false, true, 0, 0, false, 3000 );
    ...
    { // High speed LoRa channel
        ...
        Radio.SetTxConfig( MODEM_LORA, txPower, 0, 1, datarate, 1, 8, false, true, 0, 0, false, 3000 );
        TxTimeOnAir = Radio.TimeOnAir( MODEM_LORA, LoRaMacBufferPktLen );
    }
    else
    { // Normal LoRa channel
        Radio.SetMaxPayloadLength( MODEM_LORA, LoRaMacBufferPktLen );
        Radio.SetTxConfig( MODEM_LORA, txPower, 0, 0, datarate, 1, 8, false, true, 0, 0, false, 3000 );
        TxTimeOnAir = Radio.TimeOnAir( MODEM_LORA, LoRaMacBufferPktLen );
    }
#elif defined( USE_BAND_915 ) || defined( USE_BAND_915_HYBRID )
    Radio.SetMaxPayloadLength( MODEM_LORA, LoRaMacBufferPktLen );
    ...
    { // High speed LoRa channel BW500 kHz
        Radio.SetTxConfig( MODEM_LORA, txPower, 0, 2, datarate, 1, 8, false, true, 0, 0, false, 3000 );
        TxTimeOnAir = Radio.TimeOnAir( MODEM_LORA, LoRaMacBufferPktLen );
    }
    else
    { // Normal LoRa channel
...
LoRaMacStatus_t LoRaMacInitialization( LoRaMacPrimitives_t *primitives, LoRaMacCallback_t *callbacks )
{
    ...
    RadioWakeUpTime = Radio.Init( &RadioEvents );
...
LoRaMacStatus_t LoRaMacMibGetRequestConfirm( MibRequestConfirm_t *mibGet )
{
    ...
    switch( mibGet->Type )
    {
        ...
        case MIB_JOIN_DATARATE:
        {
            mibGet->Param.InitialJoinDatarate = InitialJoinDatarate;
            break;
        }
        default:
...
LoRaMacStatus_t LoRaMacMibSetRequestConfirm( MibRequestConfirm_t *mibSet )
{
    ...
    switch( mibSet->Type )
    {
        ...
        case MIB_JOIN_DATARATE:
        {
        	InitialJoinDatarate = mibSet->Param.InitialJoinDatarate;
        	break;
        }
        default:
            status = LORAMAC_STATUS_SERVICE_UNKNOWN;
...
LoRaMacStatus_t LoRaMacMlmeRequest( MlmeReq_t *mlmeRequest )
{
    ...
    switch( mlmeRequest->Type )
    {
        case MLME_JOIN:
        {
            ...
            JoinRequestTrials++;
            if ( JoinRequestTrials<5)
            	LoRaMacParams.ChannelsDatarate = InitialJoinDatarate;
            else
            	LoRaMacParams.ChannelsDatarate = AlternateDatarate( JoinRequestTrials );

            LoRaMacCallbacks->JoinRequestCB(JoinRequestTrials);

        	// GIMASI : is it first JoinRequest
            if (LoraMacFirstJoinTimeStamp == 0 );
            	LoraMacFirstJoinTimeStamp = TimerGetElapsedTime( 0 );
